...
float nem_lichniy()
{
    ...
    char *sklon_po_pad_nem_dat[9]={"Mir", "Dir", "Ihm", "Ihr", "Ihm", "Uns", "Euch", "Ihnen"};
    char *sklon_po_pad_rus_dat[9]={"Мне", "Тебе", "Ему", "Ей", "Ему", "Нам", "Вам", "Им"};
    char *sklon_po_pad_nem_akk[9]={"Mich", "Dich", "Ihn", "Sie", "Es","Uns", "Euch", "Sie"};
    char *sklon_po_pad_rus_akk[9]={"Меня", "Тебя", "Его", "Ее", "Его", "Нас", "Вас", "Их"};
    ...
    for(i=0; i<8; i++)
    free(mmm[i]);
    printf("    Вы рассмотрели личные местоимения в именительном падеже\n    Вы можете изучить склоение личных местоимений в других падежах\n    Для этого напишите:\"next\", для выхода введите любой символ\n");
    scanf("%s", vibor);
    system("clear");
    if(!strcmp(vibor, "next"))
        {
        printf(" В немеком языке существует три падежа:\n Именительный- Normativ\n Дательный- Dativ\n Родительный- Akkusativ\n Мы с Вами рассмотрели склонение личного местоимения в именительном падеже\n Сначала мы рассмотрим Dativ, после Akkusativ\n");
        printf("\v              Dativ\n");
        printf("      ------------------------\n");
	    for(i=0; i<8; i++){
	          printf("      | %-5s\t->\t%-5s|   \n", sklon_po_pad_rus_dat[i], sklon_po_pad_nem_dat[i]);}
            printf("      ------------------------\v\n     Записали и запомнили?\n     Когда вы будете готовы, нажмите клавишу \"ENTER\"\n");
            getchar();
            getchar();
            system("clear"); 
        ...
        for(i=0; i<8; i++)
        {
            printf("%s\t->\tПравильно:%s\t = \tПеревод:%s\n", mmm[i], sklon_po_pad_nem_dat[i], sklon_po_pad_rus_dat[i]);
        }
        n=0; k=0;
        for(i=0;i<8;i++)
        {
        ...
        system("clear");
        ///////////////////////////////////////////////////////////////////////////////////
        printf(" Пришло время изучить личные местоимения в склоение Akkusativ\n");
	    printf("\v              Akkusativ\n      ------------------------\n");
	    for(i=0; i<8; i++){
	          printf("      | %-5s\t->\t%-5s|   \n", sklon_po_pad_rus_akk[i], sklon_po_pad_nem_akk[i]);}
            printf("      ------------------------\v\n     Записали и запомнили?\n     Когда вы будете готовы, нажмите клавишу \"ENTER\"\n");
            getchar();
            system("clear"); 
            printf("        Подготовьтесь к написанию иностранных слов!\n");
        ...
        for(i=0; i<8; i++)
        {
            printf("%s\t->\tПравильно:%s\t = \tПеревод:%s\n", mmm[i], sklon_po_pad_nem_akk[i], sklon_po_pad_rus_akk[i]);
        }
        n=0; k=0;
        for(i=0;i<8;i++)
        {
